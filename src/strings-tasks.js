...
function reverseWords(str) {
  const strToArrayWords = str.split(' ');
  return strToArrayWords
    .map((el) => {
...
function invertCase(str) {
  const strToArrayLetters = str.split('');

  return strToArrayLetters
    .map((letter) => {
      let updatedLetter = letter;
      if (letter.match('[a-z]')) {
        updatedLetter = letter.toUpperCase();
      } else if (letter.match('[A-Z]')) {
        updatedLetter = letter.toLowerCase();
      }
...
function extractNameFromTemplate(value) {
  const startNameIntoTemplate = value.indexOf(', ') + 2;
  const endNameIntoTemplate = value.indexOf('!');

  return value.substring(startNameIntoTemplate, endNameIntoTemplate);
}
...
function unbracketTag(str) {
  const startNameIntoTemplate = str.indexOf('<') + 1;
  const endNameIntoTemplate = str.indexOf('>');

  return str.substring(startNameIntoTemplate, endNameIntoTemplate);
}
...
function encodeToRot13(str) {
  const strIntoArrLetters = str.split('');
  return strIntoArrLetters
    .map((letter) => {
